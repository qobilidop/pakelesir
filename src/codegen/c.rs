...
pub fn generate_ebpf(ir: &pb::Ir) -> Result<String> {
    ...
    writeln!(w, " * Entry contract (rbpf raw VM): mem = 8-byte LE bit_len ++ packet.")?;
    ...
    writeln!(w)?;
    writeln!(w, "uint64_t pk_entry(void *mem, uint64_t mem_len) {{")?;
    writeln!(w, "  if (mem_len < 8) return 0xFFULL << 56;")?;
    writeln!(w, "  const uint8_t *m = (const uint8_t *)mem;")?;
    writeln!(w, "  uint64_t bit_len = 0;")?;
    writeln!(w, "  uint32_t i;")?;
    writeln!(w, "  for (i = 0; i < 8; i++) bit_len |= (uint64_t)m[i] << (8 * i);")?;
    writeln!(w, "  if (bit_len > (mem_len - 8) * 8) return 0xFFULL << 56;")?;
